...
namespace ProfitRobots.TradeScriptConverter.Generators.MQL5.Tests
{
    ...
    public class BaseTests
    {
        ...
        [TestMethod]
        public void OscillatorHeader()
        {
            var script = Base.CreateOscillatorEmptyScript();
            string code = IndicatorGenerator.Generate(script);
            var reference = System.IO.File.ReadAllText(System.IO.Path.Combine("ref", "BaseTests", "OscillatorHeader.mq5"));
            Verifier.CompareLineByLine(code, reference);
        }
    }
}
